...
> **On Slack:** delivery is *native* to upstream opensre — we did not build it. We only added a small wire-up (`_post_result_to_slack`) so investigations triggered by the `/azure-alert` webhook also post to Slack (upstream auto-delivers from the CLI/normal flow, not the headless alert path), reusing upstream's `send_slack_report` unchanged. We also tweaked the RCA report formatting (`app/delivery/.../report.py`) and removed the Discord path.
